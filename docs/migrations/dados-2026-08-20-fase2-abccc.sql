...
declare
  ...
  v_linhas jsonb[] := '{}';   -- CORREÇÃO: acumula as linhas pra um único INSERT no fim
begin
  if not public.sou_staff_mimba() then raise exception 'acesso negado'; end if;
  ...
  for v_item in select value from jsonb_array_elements(v_dm) loop
    ...
    v_primeira := false;
    v_n := v_n + 1;
    v_linhas := v_linhas || v_obj;   -- CORREÇÃO: era um INSERT por linha aqui dentro
  end loop;

  -- CORREÇÃO (incidente 19/08): um único INSERT ... SELECT no lugar de ~84 mil INSERTs
  -- individuais dentro do loop plpgsql. Semântica idêntica — `linha` continua sendo
  -- v_base + a posição na ordem de leitura — mas o custo de CPU e de WAL cai de ordem.
  if array_length(v_linhas, 1) > 0 then
    insert into public.dados_registros (dataset_id, linha, dados)
    select p_dataset, v_base + ord, elem
      from unnest(v_linhas) with ordinality as t(elem, ord);
  end if;
